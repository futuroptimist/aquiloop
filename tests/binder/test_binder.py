...
class CatalogTests(unittest.TestCase):
    def placeholder_fixture(self, base):
        (base / "assets").mkdir()
        (base / "assets" / "placeholder.txt").write_text("synthetic")
        record = {
            "id": "fixture-placeholder", "path": "assets/placeholder.txt",
            "kind": "placeholder", "subjects": ["synthetic fixture"],
            "alt": "Visible synthetic placeholder.",
            "caption": "Synthetic placeholder.",
            "source": {"photographer": "not applicable",
                       "provenance": "generated test fixture", "rights": "unknown"},
        }
        catalog = {"schema_version": 1, "assets": [record]}
        (base / "assets.json").write_text(json.dumps(catalog))
        (base / "page.tex").write_text(
            "% binder-placement hero fixture-placeholder; synthetic fixture\n"
        )
        return catalog

    def test_production_entries_select_owner_photos_without_details(self):
        expected = {
    ...
    def test_duplicate_ids_and_unknown_selection_are_rejected(self):
        cases = (
            (lambda d: d["assets"].append(d["assets"][0].copy()), "fixture-placeholder", "duplicate"),
            (lambda d: None, "missing-id", "unknown asset"),
        )
        for mutation, selected_id, message in cases:
            with tempfile.TemporaryDirectory(dir=ROOT / "binder" / "entries") as name:
                base = Path(name)
                candidate = self.placeholder_fixture(base); mutation(candidate)
    ...
    def test_source_type_and_asset_kind_are_validated(self):
        for invalid_source, message in ((True, "source metadata must be an object"), (False, "unsupported kind")):
            with tempfile.TemporaryDirectory(dir=ROOT / "binder" / "entries") as name:
                base = Path(name)
                candidate = self.placeholder_fixture(base)
                if invalid_source:
                    candidate["assets"][0]["source"] = "invalid"
                else:
                    candidate["assets"][0]["kind"] = "image"
                (base / "assets.json").write_text(json.dumps(candidate))
                (base / "page.tex").write_text("% binder-placement hero fixture-placeholder; test\n")
                with self.assertRaisesRegex(ValueError, message):
                    build_binder.load_entry(base.name, "draft")
    ...
    def test_final_requires_positive_rights_review_and_matching_aspect(self):
        data = {"schema_version": 1, "assets": [{
            "id": "fixture-photo", "path": "assets/photo.jpg", "kind": "photograph",
            "subjects": ["synthetic fixture"], "alt": "Synthetic fixture.",
            "caption": "Synthetic fixture.", "source": {
                "photographer": "test suite", "provenance": "generated test fixture",
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / "binder" / "entries") as name:
            ...
            (base / "page.tex").write_text("% binder-placement hero fixture-photo; test\n")
...
class AssemblyTests(unittest.TestCase):
    ...
    def test_watering_log_and_manifest_build_with_required_text_and_order(self):
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            for page_number, (page, title) in enumerate(zip(reader.pages, expected)):
                ...
                if title != "Watering & aquarium log":
                    ...
                    actual = page.images[0].image.convert("RGB")
                    distances = {}
                    for slug in slugs:
                        source = ROOT / "binder" / "entries" / slug / "assets" / "overview.jpg"
                        with Image.open(source) as expected_image:
                            difference = ImageChops.difference(
                                actual, expected_image.convert("RGB")
                            )
                        distances[slug] = sum(ImageStat.Stat(difference).rms)
                    self.assertEqual(min(distances, key=distances.get), slugs[page_number])
