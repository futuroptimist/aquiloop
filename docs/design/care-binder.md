...
- Every source record must be cited by identity or card content. A source kept
  solely for research context must instead declare `background_only: true`; do
  not use that flag to retain speculative sources.
