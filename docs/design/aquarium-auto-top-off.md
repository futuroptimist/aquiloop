...
**Status:** Design with a Phase 0 Arduino experiment; physical validation is pending.
...
- Using Wi-Fi, Kubernetes, Prometheus, Grafana, Alertmanager, or PagerDuty in a
  local shutdown path.
- Implementing later-phase pump control, installation, dashboards, manifests,
  or cluster resources as part of Phase 0.
...
**Classification: SUPERVISED, LOW-ENERGY, DRY-BENCH EXPERIMENT ONLY.** Phase 0
covers only the reusable ESP32-S3, a protected SST input, one external LED,
serial output, and supervised validation. The runnable sketch is
[`firmware/auto_top_off/experiments/sst_led/sst_led.ino`](../../firmware/auto_top_off/experiments/sst_led/sst_led.ino),
and its practical entry point is the adjacent
[experiment README](../../firmware/auto_top_off/experiments/sst_led/README.md).

The Phase 0 estimated hardware range is **$45–$75**, including the reusable
ESP32 and SST sensor. It adds no actuator, installation, observability, or
structural part.
...
- [x] The sketch compiles with Arduino-ESP32 3.3.8 for the documented
  ESP32-S3-DevKitC-1-N8R8 configuration.
- [ ] A powered dry sensor produces HIGH and LED OFF.
- [ ] A wet tip produces LOW and LED ON.
- [ ] Sensor power OFF produces the documented conservative LOW state.
- [ ] At least five physical wet/dry cycles are recorded without unexpected
  transitions.
- [ ] If a multimeter is available, recorded GPIO4 voltage never exceeds 3.6 V.
- [ ] Wiring and observations are recorded in the validation table.

Compilation passed in the documented environment. All physical exit criteria
remain pending until a person performs and records the supervised procedure.

### 8.2 Phase 1 - supervised minimal prototype

Phase 1 builds on the verified Phase 0 protected input path and introduces
pump-driving plus the first supervised, bounded top-off behavior.
...
Approximately **$60–$190 incremental** beyond the reusable Phase 0 hardware,
for a **$105–$265 cumulative** estimate: ESP32-S3 (1, $10-$25), SST (1,
...
| --- | --- | ---: | ---: |
| 0 | ESP32-S3, protected SST input, LED, divider, bench validation | $45–$75 | $45–$75 |
| 1 | Pump/driver, listed supply, controls, tubing, PLA fixtures; reuses Phase 0 ESP32/SST path | $60–$190 | $105–$265 |
...
The Phase 0 experiment paths below now exist; the remaining entries are planned future paths:
...
firmware/auto_top_off/
  experiments/sst_led/
    ...
    sst_led.ino                         # Phase 0 Arduino sketch
