...
**Status:** Phase 0 experiment implemented; physical validation pending.
...
- Using Wi-Fi, Kubernetes, Prometheus, Grafana, Alertmanager, or PagerDuty in a
  local shutdown path.
- Production firmware, pump-control hardware, dashboards, manifests, or cluster
  resources in Phase 0.
...
VALIDATION PENDING.** Phase 0 covers only the ESP32, protected SST input,
external LED, serial output, and supervised bench validation. Its estimated
hardware range is **$45–$75**, including the reusable ESP32 and SST sensor. The
committed Arduino sketch is
...
- [ ] The sketch compiles for the documented ESP32-S3-DevKitC-1-N8R8
  configuration using Arduino-ESP32 core 3.3.8.
- [ ] A dry sensor physically produces HIGH and LED OFF.
- [ ] A wet sensor physically produces LOW and LED ON.
- [ ] Sensor power OFF physically produces the conservative LOW state.
- [ ] At least five wet/dry cycles are recorded without unexpected transitions.
- [ ] If a multimeter is available, measured GPIO4 never exceeds 3.6 V.
- [ ] The final wiring and all observations are recorded.

Compilation is a software exit criterion; physical exit criteria remain pending
until a person performs the procedure and records results. Phase 1
builds on this verified input path and introduces pump driving and the first
supervised top-off behavior.
...
Phase 1 incremental cost is approximately $60-$190; cumulative cost including
Phase 0 remains approximately $105-$265: ESP32-S3 (1, $10-$25), SST (1,
...
| --- | --- | ---: | ---: |
| 0 | ESP32-S3, protected SST input, LED, breadboard bench test | $45-$75 | $45-$75 |
| 1 | Pump/driver, listed supply, controls, tubing, PLA fixtures | $60-$190 | $105-$265 |
...
Phase 0 experiment files now exist; the remaining paths are planned future paths:
...
firmware/auto_top_off/
  experiments/sst_led/
    ...
    README.md                           # Phase 0 reproduction guide
