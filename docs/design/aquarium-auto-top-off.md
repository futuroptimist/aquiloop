...
**Status:** Design plus Phase 0 Arduino experiment; physical validation is pending.
...
- Using Wi-Fi, Kubernetes, Prometheus, Grafana, Alertmanager, or PagerDuty in a
  local shutdown path.
- Production firmware, dashboards, manifests, or cluster resources in Phase 0.
...
Phase 0 is a supervised, low-energy, dry-bench experiment proving only a
protected binary SST input, an external LED, and change-only serial reporting.
It excludes pumps, other actuators and aquarium installation; passing it does
not authorize unattended operation. Its estimated hardware range is
**$45–$75**, including the reusable ESP32 and SST.

The experiment README is the single source of truth for the exact BOM, circuit,
safety constraints, Arduino setup, bring-up procedure, troubleshooting,
validation record, and exit criteria. Follow the
[SST + LED Phase 0 experiment](../../firmware/auto_top_off/experiments/sst_led/README.md)
from start to finish before proceeding to Phase 1. No physical result is
claimed until a person completes its validation record.
...
Phase 1 incremental cost is approximately $60-$190; cumulative cost through
Phase 1 remains approximately $105-$265: ESP32-S3 (1, $10-$25), SST (1,
...
| --- | --- | ---: | ---: |
| 0 | ESP32-S3, protected SST interface, LED, bench validation | $45-$75 | $45-$75 |
...
The Phase 0 experiment exists at the path shown; the remaining entries are planned future paths:
...
firmware/auto_top_off/
  experiments/sst_led/
    ...
    README.md                           # Phase 0 procedure and validation record
